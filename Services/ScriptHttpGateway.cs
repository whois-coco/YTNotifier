...
/// <summary>
/// 要約スクリプト（Plugins\summary.js）へ公開する、接続先を限定した通信の道具。
/// 許可した接続先（Gemini API・YouTube）以外へは一切つながらない。
/// </summary>
public sealed class ScriptHttpGateway
{
    ...
    /// <summary>
    /// 要約1回あたりの通信回数の上限。同梱スクリプトが使うのは4回
    ...
    public ScriptHttpGateway(string apiKey) => _apiKey = apiKey;

    /// <summary>
    /// スクリプトから呼ばれる唯一の外部アクセス手段。許可ホスト以外・許可外スキーム・URL として
    /// 解釈できない文字列、および通信回数の上限に達した以降の呼び出しは、例外を投げずに
    /// status=0 の結果を返す。
    /// 同期的に完結させるため、UIスレッド以外から呼ばれる前提（保証は SummaryScriptService 側で行う）。
    /// </summary>
