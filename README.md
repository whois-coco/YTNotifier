...
│   ├── LoggerService.cs         # UIログ（チャンネル毎最新1件）+ 日別ファイル出力
...
| ファイル | 内容 |
|---|---|
| `config.json` | APIキー・テーマ・通知設定・ウィンドウサイズ等 |
| `channels.json` | 監視チャンネルリスト（順序・種別設定含む） |
...
- ドラッグ不要の **▲▼ ボタン**でリスト内の順序を変更
- チャンネル行クリックで最新動画をブラウザで開く
...
- 動画種別を `videos.list` + `liveStreamingDetails` で正確に判定
  - Short: 60秒以内、または180秒以内かつ `#Shorts` タグ付き
  - ライブ/アーカイブ: `liveStreamingDetails` フィールドが存在する動画
- **NEW バッジ**: 新着検出時にチャンネル名横に表示、クリックで消去
